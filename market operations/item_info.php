...
include_once("file_name.php"); // Файл с secertkey.
require_once("file_name.php"); // Коннект к БД.
...
$select_data = mysql_query("SELECT * FROM ******* WHERE ********='$user_steamid'");
$i=0;
while ($row = mysql_fetch_assoc($select_data)) {	
	$item_market_name[$i] = $row['************'];
	$sell_price[$i] =$row['*********'];
	$buy_price[$i] =$row['**********'];
	$classid_instanceid[$i] =$row['***********'];
	$i++;
}
