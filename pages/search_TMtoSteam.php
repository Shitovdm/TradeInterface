...
<div class="io" id="io">
    ...
    <script>
        ...
        function sortingBy(collomn, type) {
            ...
            for (i = 0; i < rows - 1; i++) {
                var tr1 = table.getElementsByTagName("td")[collomn + (i * 15)];
                array[i] = parseFloat(tr1.innerHTML);
            }
            ...
            for (i = 0; i < sortArray.length; i++) {
                for (j = 0; j <= sortArray.length; j++) {
                    ...
                    tdi = table.getElementsByTagName("td")[collomn + (j * 15)];
                    final = parseFloat(tdi.innerHTML);//Это число в текущей строке.
                    if ((final == sortArray[i]) && (flagArray[j] != 1)) {
                        flagArray[j] = 1;
                        document.getElementById('result_table').innerHTML += tri.innerHTML;
                        break;
                    }
                }
            }
        ...
        function re_login() {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    add_to_start("log", resp);
                    $("#STtoTM_log").append("<p>" + response.status + "</p>");
                }
            });
        }
        ...
        var withdrawel_FLAG = true;
        var steamsell_FLAG = true;
        var refresh_activeItems_FLAG = true;

        var withdrawel_counter = 0;
        ...
        setInterval(function () {
            if (withdrawel_FLAG && refresh_activeItems_FLAG) {
                if ($('#activate_auto_loading').prop('checked')) {
                    refresh_activeItems_CSGOTM();
                }
            }

            if (withdrawel_FLAG) {
                var actionItems = new Array();
                var container = document.querySelectorAll('.active_item_CSGOTM_action b');
                for (var i = 0; i < container.length; i++) {
            ...
            if (availability_of_request()) {
                if (withdrawel_counter >= 2) {
                    steamsell_FLAG = false;
                    steamsell();
                    withdrawel_counter = 0;
                }
            }
        }, 15000);
