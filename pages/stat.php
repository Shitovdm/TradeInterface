...
//-----------Working with csgoTM cUrl---------------------//	
	...
	for($i = 0;$i<$amount_items_one_upload;$i++){
		...
		foreach($data as $element){
			//$classid = $element->i_classid;
			//$instanceid = $element->i_instanceid;
			//$stickers = $element->i_stickers;
			//$quality = $element->i_quality;
			//$name = $element->name;
...
for($i = 0;$i<$amount_items_one_upload;$i++){
	...
	//$ret .= $market_hash_name[$i] . " MIN Price TM:" . $min_price_TM[$i] . " Steam: median_price: " . $median_price_STEAM[$i] . ". lowest_price: " . $lowest_price_STEAM[$i] . ". volume:" . $volume_STEAM[$i] . ". Percent: " . $TmToSteam[$i] . "<br>";
}
//echo json_encode($ret . " " . $amount_items_one_upload);
/*
$buy_responce_log = "";
for($i = 0; $i < count($ids); $i++){

	
	$name[$i] = ($market_hash_name[$i]);
	
	$price_steam[$i] = cut_priceSteam($price_steam[$i]);
	$price_steam_real[$i] = cut_priceSteam($price_steam_real[$i]);

	
	$price_tm[$i] = number_format($price_tm[$i], 2, '.', '');
	$price_tm_order[$i] = number_format($price_tm_order[$i], 2, '.', '');
	
	
	

	for($j = 0; $j < count($ids); $j++){
		$link_tm[$j] = "https://csgo.tm/item/" . str_replace("_", "-", $ids[$j]);;
	}
	
	if($price_tm[$i] == 0){
		$price_tm[$i] = "Not";
		$profit[$i] = "Not";
		$profit_persent[$i] = "Not";
		$market_hash_name[$i] = "null";
	}
...
/*
//	Покупаем автоматически только вещи с профтом от 40 процентов, которых в стиме в сутки продают больше 20 штук. 
//	Статтреки не берем, так как их мало покупают, этим фактором и обусловлен большой процент на них.

	if(strpos($volume_steam[$i], ",") === false){	
	}else{
		$volume_steam[$i] = str_replace("," , "" , $volume_steam[$i]);
	}
	if(($TmToSteam[$i] >= 30) && ($volume_steam[$i] >= 20) && ($auto_trade_ON == 'Yes') && ($slot[$i] == "Обыч.") && ($price_tm[$i] < 200)){
		$buy_request = set_request("https://market.csgo.com/api/Buy/". $ids[$i] . "/" . $price_tm_kopek[$i] . "/?key=" . $secretKey);
		...
		$query = "INSERT 
				  INTO bought_items 
				  (market_hash_name, sell_price, buy_price, classid_instanceid, user_steamid) 
				  VALUES 	
				  ('$market_hash_name[$i]', '$price_tm[$i]' , '$price_steam_real[$i]', '$ids[$i]' , '$steamid')";
		$result = mysql_query($query);

		//--------------------------------------------------------------------------------------------------
		$buy_responce_log = $buy_responce_log . "<p>[" . date("d.m.Y H:i:s") . "]" . " Bought Item: " . $name[$i] . " By price: " . $price_tm[$i] . " With profit: " . $TmToSteam[$i] . " Server responce:" . $buy_request . "</p>";	
	}
