...
if($action == "save"){
	file_put_contents($source_filename, '');// Очищаем файл с исходным кодом.
	$handle = fopen($source_filename, 'a');
	fwrite($handle, $source_code);//Записываем в файл исходный код
	
	
	//$text = file_get_contents($source_filename);
	...
	file_put_contents($filename, '');// Очищаем файл с предметами.
	$handle = fopen($filename, 'a');
	fwrite($handle, $write_str);//	Записываем предметы в файл.	
