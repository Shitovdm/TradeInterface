...
/*
*	Получаем значения для построения гистограммы.
*/
...
while ($row = mysql_fetch_assoc($select_data)) {	
	...
	//Для гистограммы считаем количество предметов по ценовой категории.
	//Покупка.
	if((int)$sell_price[$i] <= 1){$sell_price_0_1++;}
	if(((int)$sell_price[$i] > 1) && ((int)$sell_price[$i] <= 5)){$sell_price_1_5++;}
	if(((int)$sell_price[$i] > 5) && ((int)$sell_price[$i] <= 50)){$sell_price_5_50++;}
	...
	if((int)$sell_price[$i] >= 1000){(int)$sell_price_1000_inf++;}
	//Продажа.
	if((int)$buy_price[$i] <= 1){$buy_price_0_1++;}
	if(((int)$buy_price[$i] > 1) && ((int)$buy_price[$i] <= 5)){$buy_price_1_5++;}
	if(((int)$buy_price[$i] > 5) && ((int)$buy_price[$i] <= 50)){$buy_price_5_50++;}
	if(((int)$buy_price[$i] > 50) && ((int)$buy_price[$i] <= 250)){$buy_price_50_250++;}
	if(((int)$buy_price[$i] > 250) && ((int)$buy_price[$i] < 1000)){$buy_price_250_1000++;}
	if((int)$buy_price[$i] >= 1000){(int)$buy_price_1000_inf++;}
...
if($password == ""){
	...
	for($i = 0; $i < strlen(decode_password($password)); $i++){$stars .= "*";}
	$password = (substr(decode_password($password), 0, 4)) . $stars;
}
if($SteamGuardCode == ""){
	$SteamGuardCode = "The SteamGuardCode is missing!";
}else{
	//for($i = 0; $i < strlen(decode_this($SteamGuardCode)); $i++){$stars .= "*";}
...
$(document).ready(function(){
    $("#preloader_secret_key").hide();  //Скрываем прелоадер
